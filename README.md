...
Forge 1.20.1: Hold **Left Alt** (rebindable) and **right-click** a container to directly interact with it — **without opening the GUI**.
...
- `/br reload` — re-reads the container slot rules (json files / Configured list) without restarting.
...
| Farmer's Delight Cooking Pot | yes | input (no fuel slot) |
| Let's Do Farm & Charm Cooking Pot / Roaster | yes | input / dish slot |
| Let's Do Farm & Charm Stove | yes | input / fuel |
...
|---|---|---|
| Jade Preview | Show the extraction/deposit/restock preview in Jade | on |
| Deposit mode | Allow depositing held items into containers | on |
| Restock mode | Allow refilling existing items from your inventory | on |
| FTB Ultimine chain | Allow chain extraction/deposit/restock | on |
| Chain Container Limit | Max containers processed per chain | 64 |
| Per-container toggles | Enable/disable each supported container | on |
...
The modifier key (Left Alt) also works when **Carry On** is installed, so the two mods do not conflict on right-click.
...
| --- | --- | --- |
| `Auto-detect containers` | off | Master switch for the whole feature |
| `Auto-detect whitelist` | empty | Only the blocks listed here are auto-detected |
| `Active probe mode` | off | Adds an *Active Probe* entry to the mode wheel |
...
2. **Vanilla inventory interface.** Fallback that just asks `Container.canPlaceItem` with a few common
   items and checks `WorldlyContainer.canTakeItemThroughFace`.
...
- What a rule cannot reach is a container the game cannot address slot by slot: a block entity that
  does not implement `Container` at all, or a GUI whose slots point at a private inner container
