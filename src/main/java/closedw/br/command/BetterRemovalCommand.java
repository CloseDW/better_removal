    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        root.then(Commands.literal("reload").executes(ctx -> {
            ...
            if (!OutputSlotExtractor.isExperimentalEnabled()) {
                player.displayClientMessage(Component.translatable("betterremoval.message.rules_disabled")
                        .withStyle(ChatFormatting.RED), false);
            }
            player.displayClientMessage(Component.translatable("betterremoval.message.rules_reloaded", count)
                    .withStyle(ChatFormatting.YELLOW), false);
            return 1;
        }));

        var show = (com.mojang.brigadier.Command<CommandSourceStack>) ctx -> {
            ServerPlayer player = ctx.getSource().getPlayerOrException();
            player.displayClientMessage(ExtractionModeManager.getStateMessage(ExtractionModeManager.getState(player)), false);
            return 1;
        };
        root.then(Commands.literal("now").executes(show));
        root.executes(show);
